...
void MainWindow::setupRealtimeDataDemo(QCustomPlot *customPlot)
{
  ...
  customPlot->graph(2)->setPen(QPen(QColor(127,255,0)));
...
void MainWindow::realtimeDataSlot()
{
  ...
  if (key-lastPointKey > 1.00 /*0.00000002*/) // at most add point every 2 ms
  {
    // add data to lines:
    ...
    ui->customPlot->graph(2)->addData(key, qCos(key)+std::rand()/(double)RAND_MAX*qCos(key/0.6));
  ...
  if (key >= segundos){
      segundos += 30;
      qDebug() << "Rango actualizado a " << segundos << Qt::endl;
  }
  // make key axis range scroll with the data (at a constant range size of 8):
  ui->customPlot->xAxis->setRange(key, key, Qt::AlignRight);
  ui->customPlot->replot();
